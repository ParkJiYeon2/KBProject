...
<script type="text/javascript" src="./js/validity3.js"></script>
...
<h4><img src="./img/국민은행일러스트로고.png" width="150" height="100">　　　　　　상담 예약 수정</h4>
</div>
<div class="card-body">
	<fieldset>
		<form action="./CounselingUpdate?num=${param.num }" method="post" id="checkForm" name="checkForm">
			<div class="form-group">
			<label for="subject"> 제목 </label> <input type="text" name="subject"
				id="subject" placeholder="제목 입력" class="form-control" value="${counselingDTO.subject }">
			</div>
			<div class="form-group">
				<label for="address">주소 </label><br>
<input type="text" id="postcode" name="postcode" placeholder="우편번호" maxlength="8" value="${counselingDTO.postcode }">
<input type="button" onclick="execDaumPostcode()" value="우편번호 찾기" >
<input type="text" id="address" name="address" placeholder="주소" class="form-control" value="${counselingDTO.address }">
<input type="text" id="extraaddress" placeholder="참고항목" hidden="">
<input type="text" id="detailaddress" name="detailaddress" placeholder="상세주소" class="form-control" value="${counselingDTO.detailaddress }">
<script>
    function execDaumPostcode() {
        new daum.Postcode({
			...
			<label for="businesschoice"> 업무선택 : </label> <select
				...
				<option>${counselingDTO.businesschoice}</option>
			...
			<textarea cols="60" rows="20" name="content" id="content"
				placeholder="상담 내용" class="form-control" >${counselingDTO.content}</textarea>
			<br>
			<label for="reservation">상담예약 날짜선택 : </label> <input type="datetime-local"
				name="reservation" id="reservation" value="${counselingDTO.reservation }" required><br>
			<button type="submit" class="btn btn-primary">상담수정 하기</button>
			<button type="reset" class="btn btn-secondary" onclick="location.href='./CounselingSelect'">취소</button>
		</form>
	</fieldset>
</div>
	</div>
